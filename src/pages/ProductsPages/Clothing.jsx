...
const Clothing = ({ category, subcategory }) => {
    ...
    const toggleSubMenu = (selectedProductCategory) => {
        if (activeSubMenu === selectedProductCategory) {
            // If the same submenu is clicked again, reset the submenu and display all items for the main category
            setActiveSubMenu(null);
            setFilteredProducts(allProducts.filter(item => item.type.toLowerCase() === category.toLowerCase()));
        } else {
            // Set active submenu and filter products accordingly
    ...
    return (
        <div className="max-w-screen-2xl flex flex-col items-center mx-auto mt-2">
            ...
            <div className="w-full flex justify-center items-center pb-4 md:pb-0 px-4">
                <Breadcrumbs category={category} resetSubmenu={resetSubmenu} />
            </div>
            <div>
            </div>
                        {/* Main section */}
                ...
                <div className="w-full flex flex-col justify-start">
                            <div className="w-full flex items-center mb-4">
                                <h1 className="font-light text-lg md:text-3xl uppercase ">{selectedSexCategory}'s {category}</h1>
                            </div>
                            ...
                            <div className="flex items-center w-full gap-2 overflow-x-auto">
                                {filteredSubmenus.map((submenu) => (
                                    <div key={submenu.name}>
                                        <Link to={`/${selectedSexCategory}/${category}/${submenu.name.toLowerCase()}`}>
                                        <div
                                            className={`${
                                                activeSubMenu === submenu.name ? "bg-gray-500 text-white border-black" : "bg-transparent"
                                              } border-gray-300 border-[1px] hover:border-black duration-300 px-6 py-2 rounded-md flex items-center justify-center mx-auto`}
