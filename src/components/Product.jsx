...
const Product = (view) => {
    ...
    useEffect(() => {
        ...
        const product = allProducts.find((item) => item.title.toLowerCase().split(" ").join("") === lastSegment || item._id === lastSegment);
        ...
        <div className="w-full md:w-2/3 mx-auto flex flex-col justify-center items-center mt-28 md:mt-36">
            ...
            <div className="w-full my-5 flex flex-col md:flex-row">
                ...
                <div className="w-full hidden md:flex md:w-2/5 relative">
                    <img className="w-auto h-[450px] md:h-full object-cover" src={details?.image} alt="productImg" />
                    <div>
                        {details.isNew && (
                ...
                <p className="font-semibold">${details.price*baseQuantity}</p>
                    ...
                    <div className="flex gap-4">
                        ...
                        <button onClick={handleAddToCart} className="bg-black text-white py-3 px-6 rounded-md active:bg-gray-800">Add to cart</button>
                    </div>
