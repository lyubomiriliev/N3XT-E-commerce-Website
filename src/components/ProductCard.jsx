...
const ProductCard = ({ product, view }) => {
  ...
  return (
    <div
      className={`${
        view === "list" ? "flex group relative h-96" : "w-full group relative"
      }`}
    >
      <div
        ...
        className={`${
          view === "list"
            ? "w-2/3 h-full cursor-pointer rounded-md overflow-hidden"
            : "w-full h-60 md:h-96 cursor-pointer overflow-hidden"
        }`}
      >
        <img
          className={`${
            view === "list"
              ? "w-full h-96 object-cover border-[1px] border-gray-200 group-hover:scale-110 duration-300"
              : "w-full border-[1px] border-gray-200 h-80 rounded-tr-md rounded-tl-md md:h-full object-cover group-hover:scale-110 duration-300"
          }`}
          src={product.image}
          alt="productImage"
        />
      ...
      <div className="w-full border-[1px] px-2 flex justify-center flex-col">
        <div
          className={`${
            view === "list"
              ? "flex-col flex h-full justify-between px-2"
        ...
        >
          ...
          <div
            className={`${
              view === "list"
                ? "flex flex-col justify-start items-start relative overflow-hidden w-full"
                : "flex flex-col items-center justify-center gap-2 relative w-full text-sm"
            }`}
          >
            <div
              className={`${
        ...
        >
          {product.isNew && (
            <p className="bg-red-600 text-white rounded-tr-md rounded-br-md font-semibold uppercase px-4 py-1">
              Sale
            </p>
          )}
        </div>
